...
from PyQt6.QtCore import QLineF, QRect, QRectF, Qt
...
class ColourfulProgress(QProgressBar):
    ...
    def setLineWidth(self, width):
        ...
        """
        self._lineWidth = (
            max(int(width), 0) if isinstance(width, (int, float)) else 50
        )
    ...
    def setRadius(self, radius):
        ...
        """
        self._radius = (
            max(int(radius), 1) if isinstance(radius, (int, float)) else None
        )
    ...
    def paintEvent(self, _):
        ...
        vertical = option.direction == Qt.Orientation.Vertical  # 是否垂直
        ...
        radius = max(
            1,
            (min(width, self.width() if vertical else self.height()) // 4)
            if self._radius is None
            else self._radius,
        )

        reverse = (
            not vertical and option.direction == Qt.LayoutDirection.RightToLeft
        ) or vertical
        if inverted:
            reverse = not reverse
        ...
        if not indeterminate:
            ...
            if self._animation:
                if (
                    ...
                    == QProgressStyleAnimation.State.Stopped
                ):
                    # FIXME: 最小化后动画会停止
...
if __name__ == "__main__":
    ...
    w3.setOrientation(Qt.Orientation.Vertical)
    w3.setRange(0, 100)
    ...
    w4.setOrientation(Qt.Orientation.Vertical)
    w4.setRange(0, 100)
